...
import com.zh.xplan.ui.camera.utils.Bitmaps;
...
public class CameraController implements MediaRecorder.OnInfoListener {
                ...
                try {
                    if (cameraInfos == null) {
                        ...
                        for (int cameraId = 0; cameraId < count; cameraId++) {
                            ...
                            for (int a = 0; a < list.size(); a++) {
                                Camera.Size size = list.get(a);
                                if (!"samsung".equals(Build.MANUFACTURER) || !"jflteuc".equals(Build.PRODUCT) || "Meizu".equals(Build.BRAND) || size.width < 2048) {
                                    cameraInfo.pictureSizes.add(new Size(size.width, size.height));
                                }
                            }
                            Log.e("zh", "initCamera()  camera.release()");
                            camera.release();
                    ...
                    Log.e("zh", "initCamera Exception:" + e.getMessage());
                    e.printStackTrace();
                }
            }
        });
    ...
    public boolean takePicture(final File path, final CameraSession session, final onPictureCallBack  callback) {
        if (session == null) {
            return false;
        }
        final CameraInfo info = session.cameraInfo;
                    ...
                    try {
                        if (info.frontCamera != 0) {
                            try {
                                ...
                                bitmap = Bitmaps.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, false);
                            } catch (Throwable e) {
                                e.printStackTrace();
                            }
                        }
    ...
    public static  interface onPictureCallBack{
