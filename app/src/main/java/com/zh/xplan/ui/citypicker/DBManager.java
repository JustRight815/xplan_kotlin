...
public class DBManager {
    ...
    private static final String DB_NAME = "china_cities";
...
//        LogUtil.e("zh path1",XPlanApplication.getInstance().getDatabasePath(DB_NAME).getPath()  + File.separator);
    ...
    public void copyDBFile(){
        ...
        File dbFile = new File(DB_PATH + ASSETS_NAME);
        if (!dbFile.exists()){
            InputStream is;
            OutputStream os;
            try {
    ...
    public List<City> getAllCities(){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + ASSETS_NAME, null);
